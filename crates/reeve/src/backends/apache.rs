...
impl Apache {
    ...
    fn render_conf(
        ...
        // Load reeve's base set, SSL when needed, and whatever extra modules
        // this server opted into (with their prerequisites). Anything missing
        // from disk is skipped rather than failing the whole render.
        let loaded = apache_modules::load_list(brew, server, needs_ssl);
        ...
        // A few opted-in modules do nothing visible without a directive of
        // their own (mod_status/mod_info need a handler location), so reeve
        // wires those up rather than leaving them loaded-but-inert. Server
        // scope, so they apply across this instance's vhosts.
        for (name, _) in &loaded {
            if let Some(extra) = apache_modules::snippet(name) {
                out.push_str(extra);
            }
        }
...
mod tests {
    ...
    /// The module tests read the real httpd's module directory, so they only
    /// mean anything where httpd is installed. Skip rather than fail elsewhere.
    fn modules_available(brew: &Brew) -> bool {
        apache_modules::modules_dir(brew).is_dir()
    }

    #[test]
    fn opted_in_modules_render_with_their_prereqs() {
        let brew = Brew {
            prefix: "/opt/homebrew".into(),
        };
        if !modules_available(&brew) {
            return;
        }
        let state = state_with_php();
        let cfg = Config::default();
        let mut srv = server();
        srv.modules = vec!["deflate".into()];
        let v = ssl_vhost();
        let out = Apache::render_conf(&srv, &[&v], &state, &cfg, &brew).unwrap();
        assert!(out.contains("LoadModule deflate_module "));
        // mod_filter provides AddOutputFilterByType and is pulled in for us.
        assert!(
            out.contains("LoadModule filter_module "),
            "deflate's prerequisite should be loaded too"
        );
        // A module nobody asked for stays out.
        assert!(!out.contains("LoadModule dav_module "));
    }

    #[test]
    fn base_modules_always_render_and_never_duplicate() {
        let brew = Brew {
            prefix: "/opt/homebrew".into(),
        };
        if !modules_available(&brew) {
        ...
        let cfg = Config::default();
        let mut srv = server();
        // Explicitly re-picking a base module must not emit it twice.
        srv.modules = vec!["rewrite".into(), "alias".into()];
        let v = ssl_vhost();
        let out = Apache::render_conf(&srv, &[&v], &state, &cfg, &brew).unwrap();
        for m in ["rewrite_module", "alias_module", "autoindex_module"] {
            assert_eq!(
                out.matches(&format!("LoadModule {m} ")).count(),
                1,
                "{m} should be loaded exactly once"
            );
        }
        // Exactly one MPM, always.
        assert_eq!(out.matches("LoadModule mpm_").count(), 1);
    }

    #[test]
    fn status_module_brings_its_own_location_block() {
        let brew = Brew {
            prefix: "/opt/homebrew".into(),
        };
        if !modules_available(&brew) {
        ...
        let cfg = Config::default();
        let mut srv = server();
        srv.modules = vec!["status".into()];
        let v = ssl_vhost();
        let out = Apache::render_conf(&srv, &[&v], &state, &cfg, &brew).unwrap();
        assert!(out.contains("SetHandler server-status"));
        assert!(out.contains("<Location /server-status>"));
        // `Require local` needs mod_authz_host's provider.
        assert!(out.contains("LoadModule authz_host_module "));
        // And nothing is emitted for a module without a snippet.
        let mut plain = server();
        plain.modules = vec!["expires".into()];
        let out = Apache::render_conf(&plain, &[&v], &state, &cfg, &brew).unwrap();
        assert!(!out.contains("SetHandler server-status"));
    }
